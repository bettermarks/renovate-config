...
if (!target || !fs.existsSync(target)) {
  throw (
    "first argument target has to be a path to a directory but does not exist, was " +
    target
  );
}
...
if (inputs.javascript) {
  ...
  const existingContent =
    fs.existsSync(npmrcFile) && fs.readFileSync(npmrcFile, "utf8");
  let write: string | false = false;
  if (!existingContent) {
    write = `${saveExact}\n`;
